...
import { putFile, getFile, listFiles } from 'blockstack';
...
class EditorCore extends Component {
 ...
 /**
  ...
  listProjectsAsWickFile = () => {
    if (!this.props.userSession || !this.props.userSession.isUserSignedIn()) {
      this.signInIsListFiles = true;
      this.openModal("SignIn");
		} else {
      var wickFiles = [];
      listFiles((fileName) => wickFiles.push(fileName)).then(() =>
      {
        this.setState({savedFiles: wickFiles}, () => this.openModal("ListFiles"));
      });
    }
  }
   ...
   */
  exportProjectAsWickFile = (callback) => {
    if (!this.props.userSession || !this.props.userSession.isUserSignedIn()) {
      this.signInIsListFiles = false;
      this.openModal("SignIn");
		} else {
      this.showWaitOverlay();

      let toastID = this.toast('Exporting project file...', 'info', {autoClose: false});
      window.Wick.WickFile.toWickFile(this.project, file => {
        ...
        .then(res => 
        {
          ...
          {
            putFile(this.project.name + '.wick', buffer)
            .then(() =>
            {
              this.updateToast(toastID, {
                type: 'success',
  ...
  onFileSelected = (fileName) => {
    this.showWaitOverlay();
    getFile(fileName).then(wickFile =>
    {
      window.Wick.WickFile.fromWickFile(wickFile, project => {
        this.setupNewProject(project);
        this.hideWaitOverlay();
  ...
  /**
   * Handle blockstack sign in.
   */
  onCloseSignIn = (redirect) => {
    if (redirect) {
      this.showWaitOverlay();
      this.autoSaveProject(() =>{
        this.project.loginWithBlockstack = true;
        var origin = window.location.origin;
        var type = this.signInIsListFiles ? 'open' : 'save';
        this.hideWaitOverlay();
        setTimeout(() => this.props.userSession.redirectToSignIn(origin + '/editor?' + type + '=true', origin + '/manifest.json', ['store_write', 'publish_data']), 0)
      })
    }
  }
